"""
Purpose: The Intent Gate (plan.md Section 14) -- a single Gemini structured
...
Fail-closed (plan.md Rule 2): if Gemini is unavailable, or its confidence
...
never raises; every Gemini failure is caught and converted to an ESCALATE
...
async def evaluate_intent(gate_input: IntentGateInput) -> IntentDecision:
    ...
    Returns:
        IntentDecision. decision is ALLOW only if Gemini classified the
    ...
    # Low confidence overrides whatever Gemini decided -- Rule 2 treats
...
def _build_prompt(gate_input: IntentGateInput) -> str:
    """Render one IntentGateInput into the plain-text prompt sent to Gemini."""
    return (
